...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<CheckPoint-response>)))
  ...
  (cl:format cl:nil "uint8                      FREE      =  0    # point is in traversable space~%uint8                      INSCRIBED =  1    # point is in inscribed space~%uint8                      LETHAL    =  2    # point is in collision~%uint8                      UNKNOWN   =  3    # point is in unknown space~%uint8                      OUTSIDE   =  4    # point is outside the map~%~%uint8                      state             # point state: FREE, INSCRIBED, LETHAL, UNKNOWN or OUTSIDE~%uint32                     cost              # cost of the cell at point~%~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'CheckPoint-response)))
  "Returns full string definition for message of type 'CheckPoint-response"
  (cl:format cl:nil "uint8                      FREE      =  0    # point is in traversable space~%uint8                      INSCRIBED =  1    # point is in inscribed space~%uint8                      LETHAL    =  2    # point is in collision~%uint8                      UNKNOWN   =  3    # point is in unknown space~%uint8                      OUTSIDE   =  4    # point is outside the map~%~%uint8                      state             # point state: FREE, INSCRIBED, LETHAL, UNKNOWN or OUTSIDE~%uint32                     cost              # cost of the cell at point~%~%~%~%"))
