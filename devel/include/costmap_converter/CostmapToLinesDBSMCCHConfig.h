...
namespace costmap_converter
{
  ...
  public:
    ...
    public:
      ...
      virtual void clamp(CostmapToLinesDBSMCCHConfig &config, const CostmapToLinesDBSMCCHConfig &max, const CostmapToLinesDBSMCCHConfig &min) const override
      {
      ...
      virtual void calcLevel(uint32_t &comb_level, const CostmapToLinesDBSMCCHConfig &config1, const CostmapToLinesDBSMCCHConfig &config2) const override
      {
        if (config1.*field != config2.*field)
          comb_level |= level;
      }

      virtual void fromServer(const ros::NodeHandle &nh, CostmapToLinesDBSMCCHConfig &config) const override
      {
        nh.getParam(name, config.*field);
      }

      virtual void toServer(const ros::NodeHandle &nh, const CostmapToLinesDBSMCCHConfig &config) const override
      {
        nh.setParam(name, config.*field);
      }

      virtual bool fromMessage(const dynamic_reconfigure::Config &msg, CostmapToLinesDBSMCCHConfig &config) const override
      {
        return dynamic_reconfigure::ConfigTools::getParameter(msg, name, config.*field);
      }

      virtual void toMessage(dynamic_reconfigure::Config &msg, const CostmapToLinesDBSMCCHConfig &config) const override
      {
        dynamic_reconfigure::ConfigTools::appendParameter(msg, name, config.*field);
      }

      virtual void getValue(const CostmapToLinesDBSMCCHConfig &config, boost::any &val) const override
      {
        val = config.*field;
      }
    };
    ...
    public:
      ...
      virtual void updateParams(boost::any &cfg, CostmapToLinesDBSMCCHConfig &top) const override
      {
        PT* config = boost::any_cast<PT*>(cfg);
